...
import {
    buildPlayerStats, getPlayerList,
    simulateDelivery, applyDelivery,
    createMatchState, getCRR, getLast5,
} from './engine.js';
...
const MY_UID   = getUID();
const MY_SLOT  = getSlot();     // 'team1' | 'team2' | null
...
function syncPhase() {
    ...
    else if (status === 'live' || status === 'toss') showMatch();
...
async function showSetup() {
    ...
    // Both ready → trigger toss (only team1 does it)
    if (myReady && oppReady && MY_SLOT === 'team1' && roomData.status !== 'live') {
        startToss();
    }
}
...
async function startToss() {
    ...
    const p1 = roomData.players.team1.playerName;
    const p2 = roomData.players.team2.playerName;
    ...
    if (MY_SLOT === winner) {
        ...
        // Wait for the toss decision in Firebase
        SIM_ROOM.child('toss/decision').on('value', async snap => {
            if (!snap.val()) return;
            const decision = snap.val(); // 'bat' | 'bowl'
...
async function endFirstInnings(state1, t1, t2) {
    ...
    const iv = setInterval(() => {
        ...
        if (n <= 0) {
            ...
            document.getElementById('inningTab2').classList.remove('active');
...
window.setSpeed = function(btn) {
    ...
    if (matchState && !matchState.complete) {
        const inningNum = roomData?.match?.inning || 1;
        // Re-run interval
