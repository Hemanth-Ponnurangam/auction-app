...
function handleRoomClick(roomId) {
    ...
    if (room.status === 'live' || (p1Filled && p2Filled)) {
        // Rejoin: restore sessionStorage slot if UID matches a player
        const myUid = sessionStorage.getItem('sim_uid');
        if (myUid) {
            if (room.players?.team1?.uid === myUid) saveUID(myUid, roomId, 'team1');
            else if (room.players?.team2?.uid === myUid) saveUID(myUid, roomId, 'team2');
        }
