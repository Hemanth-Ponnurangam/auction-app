...
// ─── PHASE BOUNDARIES ───────────────────────────────────────────
const PHASES = { PP: [0,5], MID: [6,14], DEATH: [15,19] };
 ...
 */
function buildPlayerStats(csvRows) {
 ...
 */
function getPlayerList(csvRows) {
    const players = {};
    for (const r of csvRows) {
        if (r.batter) {
...
function createMatchState(team1, team2, team1BatsFirst) {
    const battingTeam  = team1BatsFirst ? team1 : team2;
    const fieldingTeam = team1BatsFirst ? team2 : team1;

    // Safety: ensure squads are arrays so downstream code never crashes on
    // an undefined squad[0] / squad[1] when a team joined without setting up.
    const batSquad  = Array.isArray(battingTeam.squad)  ? battingTeam.squad  : [];
    const bowlSquad = Array.isArray(fieldingTeam.squad) ? fieldingTeam.squad : [];

    const striker    = batSquad[0] ? { name: batSquad[0].name,  runs: 0, balls: 0, fours: 0, sixes: 0 }
                                   : { name: 'Batter 1',        runs: 0, balls: 0, fours: 0, sixes: 0 };
    const nonStriker = batSquad[1] ? { name: batSquad[1].name,  runs: 0, balls: 0, fours: 0, sixes: 0 }
 ...
 */
function simulateDelivery(state, stats) {
 ...
 */
function applyDelivery(state, ev) {
    // Update bowler state
