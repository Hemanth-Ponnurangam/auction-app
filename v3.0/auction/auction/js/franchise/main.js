...
function updateLiveUI(data, amIWinning) {
    ...
    if (data.highest_bidder !== '-' && data.highest_bidder !== 'Base Price' && data.highest_bidder !== '') {
        let tColor  = state.allRegisteredTeams[data.highest_bidder]?.color || '#007bff';
        let logoSrc = (state.globalTeamLogos && state.globalTeamLogos[data.highest_bidder]) || '';
        if (logoSrc) {
            // Show team logo circle with glowing colour border
            leaderEl.innerHTML =
                `<img src="${logoSrc}" alt="${esc(data.highest_bidder)}"` +
                ` style="width:36px;height:36px;object-fit:contain;border-radius:50%;` +
                `padding:3px;box-sizing:border-box;background:#0a0a0f;` +
                `border:2px solid ${tColor};` +
                `box-shadow:0 0 10px ${tColor}80,0 0 3px ${tColor}60;display:block;">`;
            leaderEl.style.cssText += ';padding:0!important;border:none!important;border-radius:50%!important;overflow:hidden!important;line-height:1!important;';
        } else {
            // Fallback: colour-coded text badge
            leaderEl.innerHTML = '';
            leaderEl.textContent = data.highest_bidder;
            leaderEl.style.color = tColor;
            leaderEl.style.padding = '4px 12px';
            leaderEl.style.border  = `1px solid ${tColor}`;
            leaderEl.style.borderRadius = '6px';
        }
        leaderEl.style.display      = 'inline-flex';
        leaderEl.style.alignItems   = 'center';
        leaderEl.style.verticalAlign = 'middle';
    } else {
...
window.switchTab = function(name, el) {
    ...
    // Show star filter button only on deck tab; reset filter when leaving deck
    ...
    if (starBtn) {
        if (name === 'deck') {
            ...
            // Reset star filter state so it doesn't bleed into other tabs
            if (_isStarFilterActive) {
                _isStarFilterActive = false;
                starBtn.style.background = '#111';
                starBtn.style.borderColor = '#333';
                starBtn.style.boxShadow = 'none';
                starBtn.style.color = '#888';
            }
        }
    }
};
