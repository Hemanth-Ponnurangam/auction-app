...
export function renderDeckList(containerId, activeSet, searchQuery, roleFilter, watchlistSet = null, isAuctioneer = false, isStarFilterOnly = false) {
    ...
    let filtered = state.playerPool.map((p, i) => ({ p, i })).filter(item => {
        ...
        // Star filter takes priority — show only watchlisted players
        if (isStarFilterOnly && watchlistSet) {
            if (!watchlistSet.has(item.p.name)) return false;
            return true; // skip other filters when star mode is active
        }
    ...
    filtered.forEach(item => {
        ...
        let roleStr = item.p.role ? `<span style="font-size:9px; background:#222; padding:2px 5px; border-radius:4px; color:#0dcaf0; white-space:nowrap;">${esc(item.p.role)}</span>` : '';
        let isOv = !['india','indian','ind'].includes((item.p.nationality || 'Indian').trim().toLowerCase());
        let plane = isOv ? '✈️' : '';
        let priceStr = `<span style="font-size:9px; color:#888; white-space:nowrap;">₹${(item.p.base_price/CRORE).toFixed(2)}Cr</span>`;

        html += `<div class="list-item"><div class="item-info" style="flex-direction:row; align-items:center; gap:5px; flex:1; min-width:0;"><span class="item-name" style="flex:1; min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${esc(item.p.name)} <span style="font-size:10px;">${plane}</span></span>${priceStr}${roleStr}</div>${btn}</div>`;
    });
    list.innerHTML = html;
}
...
export function renderUnsoldList(containerId, searchQuery, isAuctioneer = false) {
    ...
    filtered.forEach(item => {
        ...
        let priceStr = `<span style="font-size:9px; color:#888; white-space:nowrap;">₹${(item.p.base_price/CRORE).toFixed(2)}Cr</span>`;
        html += `<div class="list-item" style="border-left-color:#dc3545;"><div class="item-info" style="flex-direction:row; align-items:center; gap:5px; flex:1; min-width:0;"><span class="item-name" style="flex:1; min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${esc(item.p.name)}</span>${priceStr}</div>${btn}</div>`;
    });
    list.innerHTML = html;
}
