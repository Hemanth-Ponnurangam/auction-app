...
function _clearSoldUI() {
    ...
    // Native player-header flip card
    let inner = document.getElementById('playerHeaderFlipInner');
...
function triggerSoldAnimation(teamName, teamColor, soldPrice) {
    ...
    // Logo comes from global_teams (not teams_auth)
    let logoSrc = (state.globalTeamLogos && state.globalTeamLogos[teamName]) || '';

    function _showFlip() {
        let headerInner = document.getElementById('playerHeaderFlipInner');
        if (!headerInner) return;
        headerInner.classList.remove('flipped');

        // Flip 1: info → logo at 1.6s
        _soldFlipTimer = setTimeout(() => {
            // Force animation restart: remove then add in next frame
            headerInner.classList.add('flipped');
            // Briefly force a style recalc so CSS animations replay on back face
            void headerInner.offsetHeight;

            // Flip 2: logo → info at 3.4s
            setTimeout(() => {
                headerInner.classList.remove('flipped');
                // Remove photo stamp and reset at 5.2s
