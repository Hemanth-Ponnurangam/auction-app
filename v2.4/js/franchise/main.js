...
function _triggerFlagCoinFlip(teamName, teamColor) {
    ...
    if (!inner || !soldEl) return;

    // Set team color on badge border
    if (badge) badge.style.borderColor = teamColor || '#22c55e';

    // Mid-flip: swap to SOLD face
    inner.classList.remove('flipping');
    void inner.offsetWidth; // reflow
    
    // Trigger animation — swap face at midpoint (scaleX=0)
    inner.style.transition = 'transform 0.28s cubic-bezier(0.55,0,0.45,1)';
    inner.style.transform = 'scaleX(0.04)';
    setTimeout(() => {
        if (frontEl) frontEl.style.opacity = '0';
        if (soldEl)  soldEl.classList.add('show');
        if (soldEl) soldEl.style.background = `radial-gradient(circle, ${teamColor || '#0a2a0a'}, #020602)`;
        inner.style.transition = 'transform 0.28s cubic-bezier(0.55,1,0.45,1)';
            ...
            Object.keys(groups).forEach(label => {
                ...
                groups[label].forEach(({ code, team: t }) => {
                    const logoHtml = t.logo
                        ...
                        : '';
                    html += `<button class="team-btn" style="background:${esc(t.color || '#222')}; display:flex; flex-direction:column; align-items:center; justify-content:center; padding:8px; gap:2px;" onclick="prepareLogin('${esc(code)}','${esc(t.color || '#007bff')}')">
                        ${logoHtml}
...
function attachFirebaseListeners() {
    ...
    db.ref('global_teams').on('value', snap => {
        ...
        let myLogoBanner2 = document.getElementById('myTeamLogoBanner');
        if (myLogoBigEl2 && state.myTeamName) {
            let src = state.globalTeamLogos[state.myTeamName] || '';
            if (src) { myLogoBigEl2.src = src; if (myLogoBanner2) myLogoBanner2.style.display = 'flex'; }
            else { if (myLogoBanner2) myLogoBanner2.style.display = 'none'; }
        }
...
function updateLiveUI(data, amIWinning) {
    ...
    if (pIdx >= 0 && state.playerPool.length > 0 && data.auction_state !== 'idle') {
        ...
        if (p) {
            ...
            // Show national_board + franchise as combined pill, or each separately
            const boardText   = (p.national_board || '').trim();
            const franchiseText = (p.franchise || '').trim();
            if (boardText || franchiseText) {
                const combined = [boardText, franchiseText].filter(Boolean).join(' / ');
                tags.textContent = combined;
...
function updateMyTeamUI() {
    try {
        ...
        document.getElementById('myTeamDisplay').textContent   = state.globalTeamNames?.[state.myTeamName] || state.myTeamName;
        // Show franchise logo banner at top of panel
        let myLogoBigEl = document.getElementById('myTeamLogoBig');
        let myLogoBanner = document.getElementById('myTeamLogoBanner');
        let myLogoSrc = (state.globalTeamLogos && state.globalTeamLogos[state.myTeamName]) || '';
        if (myLogoBigEl) {
            if (myLogoSrc) {
                myLogoBigEl.src = myLogoSrc;
                if (myLogoBanner) myLogoBanner.style.display = 'flex';
            } else {
                if (myLogoBanner) myLogoBanner.style.display = 'none';
            }
        }
