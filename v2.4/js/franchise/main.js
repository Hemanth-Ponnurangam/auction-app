...
function _triggerFlagCoinFlip(teamName, teamColor) {
    let badge        = document.getElementById('flagCircle');
    let inner        = document.getElementById('flagCircleInner');
    let frontEl      = document.getElementById('flagFaceFront');
    let soldEl       = document.getElementById('flagFaceSold');
    let soldLogoImg  = document.getElementById('flagFaceSoldLogoImg');
    let soldText     = document.getElementById('flagFaceSoldText');
    if (!badge || !inner || !soldEl) return;

    // ── Prep sold face: franchise logo or team initials ──────────────────
    let logoSrc = (state.globalTeamLogos && state.globalTeamLogos[teamName]) || '';
    soldEl.style.background = `radial-gradient(circle at 40% 40%, ${teamColor || '#0a2a0a'} 0%, #020602 100%)`;
    if (soldLogoImg) {
        if (logoSrc) {
            soldLogoImg.src = logoSrc;
            soldLogoImg.style.display = 'block';
            if (soldText) soldText.style.display = 'none';
        } else {
            soldLogoImg.style.display = 'none';
            if (soldText) {
                soldText.textContent = teamName.slice(0, 3).toUpperCase();
                soldText.style.color = teamColor || '#22c55e';
    ...
    badge.style.borderColor = teamColor || '#22c55e';
    if (frontEl) frontEl.style.opacity = '1';
    soldEl.classList.remove('show');
    void badge.offsetWidth; // reflow

    // ── Phase 1: Rise up fast, decelerates at apex (220 ms) ──────────────
    badge.style.transition = 'transform 220ms cubic-bezier(0.3, 0, 0.1, 1)';
    badge.style.transform  = 'translateY(-22px) scale(1.22)';

    // ── Phase 2: Fast flip first half at apex (starts 180 ms) ────────────
    setTimeout(() => {
        inner.style.transition = 'transform 120ms cubic-bezier(0.55, 0, 0.45, 1)';
        inner.style.transform  = 'scaleX(0.04)';
    }, 180);

    // ── Phase 3: Content swap at midpoint → slow reveal (120+120 ms) ─────
    setTimeout(() => {
        if (frontEl) frontEl.style.opacity = '0';
        soldEl.classList.add('show');
        inner.style.transition = 'transform 200ms cubic-bezier(0.15, 0.85, 0.45, 1)'; // slow in, ease out
        inner.style.transform  = 'scaleX(1)';
    }, 180 + 120);

    // ── Phase 4: Brief pause at apex, then descend fast ───────────────────
    setTimeout(() => {
        badge.style.transition = 'transform 230ms cubic-bezier(0.4, 0, 0.6, 1)';
        badge.style.transform  = 'translateY(0) scale(1)';
    }, 180 + 120 + 200 + 60);
}

function _resetFlagCircle() {
    let inner   = document.getElementById('flagCircleInner');
    let frontEl = document.getElementById('flagFaceFront');
    let soldEl  = document.getElementById('flagFaceSold');
    let badge   = document.getElementById('flagCircle');
    ...
    if (frontEl) { frontEl.style.opacity = '1'; }
    if (soldEl)  { soldEl.classList.remove('show'); }
    if (soldLogoImg) { soldLogoImg.src = ''; soldLogoImg.style.display = 'none'; }
    if (soldText)    { soldText.textContent = ''; }
}
...
function _clearSoldUI() {
    ...
    // Hide coin panel
    let panel = document.getElementById('coinFlipPanel');
...
// Single coin half-turn: squish to edge, swap face, expand back
// halfMs = duration of each half; cb called after full flip completes
function _coinFlip(disc, swapFn, halfMs, cb) {
    disc.style.transition = `transform ${halfMs}ms cubic-bezier(0.55, 0, 0.45, 1)`;
    disc.style.transform  = 'scaleX(0.04)';    // near-edge, not perfectly 0 (looks more physical)
    setTimeout(() => {
        swapFn();
        disc.style.transition = `transform ${halfMs}ms cubic-bezier(0.55, 1, 0.45, 1)`;
        disc.style.transform  = 'scaleX(1)';
        setTimeout(cb, halfMs);
    }, halfMs);
}

// Multi-flip sequence: flip n times with alternating faces, end on finalFace ('logo'|'sold')
// flips: array of {toFace:'logo'|'sold', pause:ms after flip before next}
function _coinFlipSequence(disc, faceA, faceB, flips, halfMs, onDone) {
    if (!flips.length) { if (onDone) onDone(); return; }
    let [flip, ...rest] = flips;
    _coinFlip(disc, () => {
        if (flip.toFace === 'sold') {
        ...
        setTimeout(() => _coinFlipSequence(disc, faceA, faceB, rest, halfMs, onDone), flip.pause || 0);
    });
}
...
function triggerSoldAnimation(teamName, teamColor, soldPrice) {
    ...
    // ── 0. Flag circle mini coin flip ─────────────────────────────────────
    setTimeout(() => _triggerFlagCoinFlip(teamName, teamColor), 400);

    // ── 1. Slant stamp on photo ───────────────────────────────────────────
    let photoStamp = document.getElementById('photoSoldStamp');
    if (photoStamp) photoStamp.classList.add('show');

    // ── 2. Coin flip in info area ─────────────────────────────────────────
    let panel      = document.getElementById('coinFlipPanel');
    let disc       = document.getElementById('coinDisc');
    let faceA      = document.getElementById('coinFaceLogo');  // logo
    let faceB      = document.getElementById('coinFaceSold');  // SOLD text
    let coinLogo   = document.getElementById('coinLogo');
    let coinInit   = document.getElementById('coinInitials');
    let coinTName  = document.getElementById('coinTeamName');
    let priceEl    = document.getElementById('coinSoldPrice');

    if (!panel || !disc) return;

    // Pre-populate team name & price
    if (coinTName) { coinTName.textContent = teamName; coinTName.style.color = teamColor || '#ffd700'; }
    ...
    function _prepLogo() {
        if (logoSrc) {
            coinLogo.src = logoSrc;
            coinLogo.style.display = 'block';
            if (coinInit) coinInit.style.display = 'none';
        } else {
            coinLogo.style.display = 'none';
            if (coinInit) {
                coinInit.textContent  = teamName.slice(0,3).toUpperCase();
                coinInit.style.color  = teamColor || '#ffd700';
                coinInit.style.display = 'block';
            }
        }
    }
    ...
    function _runAnimation() {
        ...
        const HALF = 280; // ms per half-flip — snappy but readable

        // Coin toss sequence: starts logo-up, spins realistically,
        // slows and settles on logo. Pattern: fast spins → slow → stop.
        // Each entry: which face shows after the flip, and pause before next flip.
        const sequence = [
            { toFace: 'sold', pause: 80  },   // fast
            { toFace: 'logo', pause: 80  },   // fast
            { toFace: 'sold', pause: 100 },   // slightly slower
            { toFace: 'logo', pause: 120 },   // slowing
            { toFace: 'sold', pause: 300 },   // pause to show SOLD clearly
            { toFace: 'logo', pause: 500 },   // pause to show logo — coin settling
            { toFace: 'sold', pause: 900 },   // slow final flip to SOLD
            { toFace: 'logo', pause: 1200 },  // settle back to logo (final resting)
        ];

        _soldT1 = setTimeout(() => {
            _coinFlipSequence(disc, faceA, faceB, sequence, HALF, () => {
                // Coin has settled — fade out after a beat
                _soldT3 = setTimeout(() => {
                    panel.classList.remove('show');
                    let ps2 = document.getElementById('photoSoldStamp');
                    if (ps2) ps2.classList.add('hide');
                    setTimeout(() => _clearSoldUI(), 350);
                }, 800);
            });
        }, 700); // initial pause to see logo before spin starts
    }

    if (logoSrc) {
        let preloader = new Image();
        preloader.onload  = () => { _prepLogo(); _runAnimation(); };
        preloader.onerror = () => { logoSrc = ''; _prepLogo(); _runAnimation(); };
        preloader.src = logoSrc;
    } else {
        _prepLogo();
        _runAnimation();
    }

    // ── 3. Card tickers on other franchise cards ──────────────────────────
            ...
            Object.keys(groups).forEach(label => {
                ...
                groups[label].forEach(({ code, team: t }) => {
                    const logoHtml = t.logo
                        ? `<img src="${esc(t.logo)}" style="height:32px; width:32px; object-fit:contain; margin-bottom:4px; filter:drop-shadow(0 0 4px rgba(255,255,255,0.3));">`
                        : '';
                    html += `<button class="team-btn" style="background-color:${esc(t.color || '#222')}; background-image:linear-gradient(145deg, rgba(255,255,255,0.18) 0%, rgba(0,0,0,0.28) 100%); display:flex; flex-direction:column; align-items:center; justify-content:center; padding:8px; gap:2px;" onclick="prepareLogin('${esc(code)}','${esc(t.color || '#007bff')}')">
                        ${logoHtml}
                        <span style="font-size:12px; font-weight:900;">${esc(code)}</span>
                    </button>`;
                });
            });
