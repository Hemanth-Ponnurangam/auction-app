...
function attachFirebaseListeners() {
    ...
    db.ref('global_teams').on('value', snap => {
        ...
        // Refresh logo in header and other UI now that we have data
        let myLogoEl = document.getElementById('myTeamLogo');
        if (myLogoEl && state.myTeamName) {
            let src = state.globalTeamLogos[state.myTeamName] || '';
            if (src) { myLogoEl.src = src; myLogoEl.style.display = 'inline-block'; }
...
function updateMyTeamUI() {
    try {
        ...
        // Show franchise logo next to team name (logos live in global_teams, not teams_auth)
        let myLogoEl = document.getElementById('myTeamLogo');
        ...
        Object.keys(state.allRegisteredTeams).forEach(t => {
            ...
            othersHtml += `
            <div class="other-team-card-hz${lgClass}" data-team="${esc(t)}" style="border-top-color:${tColor}; background: linear-gradient(160deg, rgba(${r16},${g16},${b16},0.13) 0%, #0a0a0f 55%); border-color: rgba(${r16},${g16},${b16},0.3);">
                ${bgLogoDivHtml}
                ...
                <div class="purse-val" style="color:${rCr > 0 ? '#28a745' : '#dc3545'}; font-size:17px; position:relative; z-index:2;">₹${rCr.toFixed(2)} Cr</div>
