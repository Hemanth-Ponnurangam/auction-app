...
function updateLiveUI(data, amIWinning) {
    ...
    if (pIdx >= 0 && state.playerPool.length > 0 && data.auction_state !== 'idle') {
        ...
        if (p) {
            ...
            document.getElementById('playerName').innerHTML = esc(p.name) + (isOv ? `<span class="overseas-wrap" title="${esc(p.nationality)}"><svg class="overseas-icon" viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 3a15 15 0 0 1 0 18M12 3a15 15 0 0 0 0 18M3 12h18"/></svg></span>` : '');
    ...
    try {
        ...
        function planeIcon(p) {
            return !['india','indian','ind'].includes((p.nationality || 'Indian').trim().toLowerCase()) 
                ? `<span class="overseas-wrap" title="${esc(p.nationality)}"><svg class=\"overseas-icon\" viewBox=\"0 0 24 24\" width=\"14\" height=\"14\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.8\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><circle cx=\"12\" cy=\"12\" r=\"9\"/><path d=\"M12 3a15 15 0 0 1 0 18M12 3a15 15 0 0 0 0 18M3 12h18\"/></svg></span>` : '';
        }
