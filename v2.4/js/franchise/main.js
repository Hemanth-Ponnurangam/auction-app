...
function triggerSoldAnimation(teamName, teamColor, soldPrice) {
    // ── 1. Center-panel flip card — double flip: SOLD → logo → SOLD ───────
    ...
    // Resolve logo safely — avoid escaping base64 through innerHTML
    ...
    if (overlay && inner) {
        ...
        // Pre-populate back face BEFORE showing overlay
        let logoImg = document.getElementById('sfoLogo');
        let initEl  = document.getElementById('sfoInitials');
        if (logoSrc) {
            ...
            preloader.onerror = () => {
                ...
                if (initEl)  { initEl.textContent = teamName.slice(0,3).toUpperCase(); initEl.style.color = teamColor || '#ffd700'; initEl.style.display = 'block'; }
            };
            preloader.src = logoSrc;
        } else {
            // No logo — show team initials
            if (logoImg) logoImg.style.display = 'none';
            if (initEl)  { initEl.textContent = teamName.slice(0,3).toUpperCase(); initEl.style.color = teamColor || '#ffd700'; initEl.style.display = 'block'; }
        }
        ...
        // Re-enable transition after reset frame
        requestAnimationFrame(() => {
            requestAnimationFrame(() => {
                ...
                // Flip 1 — front → back (logo) at 1.5s
                _soldFlipTimer = setTimeout(() => {
                    inner.classList.add('flipped');

                    // Flip 2 — back → front (SOLD) at 3.0s
                    setTimeout(() => {
                        inner.classList.remove('flipped');
                        ...
                        setTimeout(() => {
                            ...
                            setTimeout(() => overlay.classList.remove('show', 'hiding'), 600);
                        }, 1600);
                    }, 1500);
                }, 1500);
            });
    ...
    // ── 2. Ticker ON each other-team card — positioned absolute, no layout shift ──
    let playerName = state._lastSoldPlayerName || '';
    let color = teamColor || '#22c55e';
    let chipContent = `<span style="color:#22c55e;margin-right:6px;">●</span><span style="color:#fff;font-weight:700;">${esc(playerName)}</span><span style="color:#888;margin:0 8px;">→</span><span style="color:${color};font-weight:800;letter-spacing:1px;">${esc(teamName)}</span><span style="color:#ffd700;margin-left:8px;font-weight:700;">${soldPrice || ''}</span>`;
    // Repeat content for seamless loop
    let trackContent = `<span style="padding-right:60px;">${chipContent}</span>`.repeat(4);

    // Fill pre-rendered ticker slots at top of each other-team card
...
function triggerUnsoldAnimation() {
    ...
    if (overlay && inner) {
        ...
        if (stampEl) {
            ...
            stampEl.style.textShadow = '0 0 24px rgba(239,68,68,0.85)';
            stampEl.style.fontSize = 'clamp(28px, 5vw, 56px)'; // tighter for longer word
            stampEl.style.letterSpacing = '4px';
        }
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                if (stampEl2) {
                    stampEl2.textContent = 'SOLD!';
                    stampEl2.style.color = '#22c55e';
                    stampEl2.style.textShadow = '';
                    stampEl2.style.fontSize = '';
                    stampEl2.style.letterSpacing = '';
                }
