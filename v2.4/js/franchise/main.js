...
// Coin-flip helper: animates scaleX 1→0→1 with a content swap at the zero point
// duration ms for each half; calls swapFn() at the centre (scaleX=0)
function _coinFlip(disc, swapFn, halfMs, cb) {
    disc.style.transition = `transform ${halfMs}ms cubic-bezier(0.4,0,0.6,1)`;
    disc.style.transform  = 'scaleX(0)';
    setTimeout(() => {
        swapFn();
        disc.style.transition = `transform ${halfMs}ms cubic-bezier(0.4,1,0.6,1)`;
...
function triggerSoldAnimation(teamName, teamColor, soldPrice) {
    ...
    function _runAnimation() {
        ...
        const HALF = 350; // ms per half-flip

        // Short pause so logo is visible first (0.8s)
        _soldT1 = setTimeout(() => {
            ...
            _coinFlip(disc, () => {
                ...
                // Hold SOLD face for 1.4s, then flip back to logo
                _soldT2 = setTimeout(() => {
                    _coinFlip(disc, () => {
                        faceB.style.display = 'none';
                        faceA.style.display = 'flex';
                        ...
                        // Hold logo for 1s, then fade everything out
                        _soldT3 = setTimeout(() => {
...
function updateLiveUI(data, amIWinning) {
    ...
    if (pIdx >= 0 && state.playerPool.length > 0 && data.auction_state !== 'idle') {
        ...
        if (p) {
            ...
            document.getElementById('playerName').innerHTML = esc(p.name) + (isOv ? `<span class="overseas-wrap" title="${esc(p.nationality)}"><svg class="overseas-icon" viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:middle;margin-left:6px;color:#0dcaf0;filter:drop-shadow(0 0 4px rgba(13,202,240,0.6));"><path d="M21 3L3 10.5l7.5 3L18 7l-5.5 7.5 3 7.5L21 3z"/></svg></span>` : '');
    ...
    try {
        ...
        function planeIcon(p) {
            return !['india','indian','ind'].includes((p.nationality || 'Indian').trim().toLowerCase()) 
                ? `<span class="overseas-wrap" title="${esc(p.nationality)}"><svg class=\"overseas-icon\" viewBox=\"0 0 24 24\" width=\"16\" height=\"16\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.8\" stroke-linecap=\"round\" stroke-linejoin=\"round\" style=\"display:inline-block;vertical-align:middle;margin-left:4px;color:#0dcaf0;\"><path d=\"M21 3L3 10.5l7.5 3L18 7l-5.5 7.5 3 7.5L21 3z\"/></svg></span>` : '';
        }
