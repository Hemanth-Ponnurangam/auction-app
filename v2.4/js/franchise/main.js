...
function triggerSoldAnimation(teamName, teamColor, soldPrice) {
    ...
    // Inject ticker on ALL other-team cards
    let cards = document.querySelectorAll('.other-team-card-hz');
    cards.forEach(card => {
        // Remove any existing ticker on this card
        let existing = card.querySelector('.card-ticker-wrap');
        if (existing) existing.remove();

        let wrap = document.createElement('div');
        wrap.className = 'card-ticker-wrap';
        ...
        // Remove after 6s
        clearTimeout(card._tickerTimer);
        card._tickerTimer = setTimeout(() => {
