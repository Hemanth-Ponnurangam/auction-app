...
export function evaluateBidButtonStatus(amIWinning) {
    ...
    const setBtn = (cls, text, enabled) => {
        ...
        // Always clear ticker unless we're in sold state
        if (cls !== 'btn-bid-locked') {
            let t = document.getElementById('bidTickerBar');
    ...
    if (currentStatus === 'idle') {
        ...
        setBtn('btn-bid-locked', 'SOLD', false);
        bidZone.style.borderColor = 'rgba(34,197,94,.25)';
        // Show running text on ticker bar instead of replacing button
        let soldPlayer = (state.playerPool[state.liveState.current_player_index] || {}).name || '';
        let soldBidder = state.liveState.highest_bidder || '';
        let soldAmt    = state.liveState.current_bid ? '₹' + (state.liveState.current_bid / 10_000_000).toFixed(2) + ' Cr' : '';
        let chip = `<span style="color:#22c55e;">●</span> <span style="color:#fff;font-weight:700;">${soldPlayer}</span> <span style="color:#555;margin:0 6px;">·</span> <span style="color:#ffd700;font-weight:700;">${soldAmt}</span> <span style="color:#555;margin:0 6px;">·</span> <span style="color:#22c55e;font-weight:800;letter-spacing:1px;">${soldBidder}</span>`;
        let rep = `<span style="padding-right:72px;">${chip}</span>`;
        let ticker = document.getElementById('bidTickerBar');
        let track  = document.getElementById('bidTickerTrack');
        if (ticker && track) {
            track.innerHTML = rep.repeat(5);
            ticker.classList.add('show');
        }
    } else if (currentStatus === 'unsold') {
        setStatus('status-losing');  setBtn('btn-bid-unsold','UNSOLD',false);
        // Hide ticker
        let ticker2 = document.getElementById('bidTickerBar');
        if (ticker2) ticker2.classList.remove('show');
