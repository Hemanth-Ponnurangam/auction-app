...
// FIX: CSV Apostrophe Parsing — now captures dynamic stat columns
function parseCSV(text) {
    let lines = text.replace(/\r\n/g, '\n').replace(/\r/g, '\n').split('\n');
    let raw = lines[0].split(',');
    let hdrs = raw.map(h => h.trim().toLowerCase().replace(/[^a-z0-9_]/g,''));
    let rawHdrs = raw.map(h => h.trim());
    const RESERVED = new Set(['baseprice','name','set','role','franchise','nationality',
        'runs','average','avg','sr','strikerate','wickets','economy','econ','bowlingsr']);
    let dynamicStatCols = [];
    hdrs.forEach((h, i) => { if (h && !RESERVED.has(h)) dynamicStatCols.push({ key: h, label: rawHdrs[i], idx: i }); });
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (dynamicStatCols.length) {
            obj.stats = {};
            dynamicStatCols.forEach(({ key, idx }) => {
    ...
    result._statColumns = dynamicStatCols.map(c => c.key);
    result._statLabels  = dynamicStatCols.map(c => c.label);
    return result;
}
...
function updatePlayerCard(data, isIdle) {
    ...
    if (currentIndex >= 0 && state.playerPool.length > 0 && !isIdle) {
        ...
        if (p) {
            ...
            // Dynamic stats bar — reads from stat_columns setting (from CSV) or falls back to legacy
            let statsBar = document.getElementById('dynamicStatsBar');
            let statCols = state.settings.stat_columns || [];
            if (statsBar) {
                let html = '';
                if (statCols.length && p.stats) {
                    ...
                    const legacyStats = [
                        ...
                        ['WKTS', p.wickets||'-'], ['ECON', p.economy||'-'], ['B.SR', p.bowl_sr||'-']
                    ];
                    legacyStats.forEach(([label, val]) => {
                        html += `<div class="stat-box"><div class="stat-label">${label}</div><div class="stat-val">${esc(String(val))}</div></div>`;
                    });
                }
        ...
        if (statsBar) {
            ...
            let html = '';
            let statCols = state.settings.stat_columns || [];
            let cols = statCols.length ? statCols : ['RUNS','AVG','SR','WKTS','ECON','B.SR'];
            cols.forEach(col => {
                let label = col.replace(/_/g,' ').toUpperCase();
                html += `<div class="stat-box"><div class="stat-label">${esc(label)}</div><div class="stat-val">-</div></div>`;
            });
            statsBar.innerHTML = html;
        }
    }
...
function updateBudgetTracker() {
    ...
    for (let i = 0; i < totalSlots; i++) {
        if (i < keys.length) {
            ...
            let logoHtml = tData.logo ? `<img src="${esc(tData.logo)}" style="height:28px;max-width:52px;object-fit:contain;filter:drop-shadow(0 0 4px rgba(255,255,255,0.15));margin-bottom:3px;" alt="">` : '';
