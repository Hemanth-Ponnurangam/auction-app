...
export function renderDeckList(containerId, activeSet, searchQuery, roleFilter, watchlistSet = null, isAuctioneer = false, forceStarOnly = false) {
    ...
    let filtered = state.playerPool.map((p, i) => ({ p, i })).filter(item => {
        ...
        // Universal Star Bypass - Ignore Status and Set boundaries!
        if (forceStarOnly) {
            if (!watchlistSet || !watchlistSet.has(item.p.name)) return false;
            // Still allow text search if active
            if (searchQuery && !(item.p.name||'').toLowerCase().includes(searchQuery)) return false;
            return true;
        }

        // Standard Rules
        if (item.p.status === 'sold' || item.p.status === 'unsold') return false;
    ...
    filtered.forEach(item => {
        ...
        let plane = isOv ? '✈️' : '';
        let statusWarn = (item.p.status === 'sold' || item.p.status === 'unsold') ? `<span style="color:#ffc107; font-size:9px; margin-left:4px;">(${item.p.status.toUpperCase()})</span>` : '';

        html += `<div class="list-item"><div class="item-info"><span class="item-name">${esc(item.p.name)} <span style="font-size:10px;">${plane}</span> ${statusWarn}</span><span class="item-price">Base: ₹${(item.p.base_price/CRORE).toFixed(2)} Cr ${roleStr}</span></div>${btn}</div>`;
    });
    list.innerHTML = html;
}
