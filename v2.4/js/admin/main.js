...
function setupEventListeners() {
    ...
    document.getElementById('btnAddFlag')?.addEventListener('click', () => document.getElementById('addFlagModal').style.display = 'flex');
    document.getElementById('btnCloseFlag')?.addEventListener('click', () => document.getElementById('addFlagModal').style.display = 'none');
    document.getElementById('btnSubmitFlag')?.addEventListener('click', handleFlagUpload);

    document.getElementById('btnAddRoleIcon')?.addEventListener('click', () => document.getElementById('addRoleIconModal').style.display = 'flex');
    document.getElementById('btnCloseRoleIcon')?.addEventListener('click', () => document.getElementById('addRoleIconModal').style.display = 'none');
...
function renderNatFlags() {
    ...
    keys.forEach(key => {
        ...
        card.innerHTML = `
            <button onclick="window.deleteNatFlag('${esc(key)}')" style="position:absolute; top:6px; right:6px; background:rgba(220,53,69,0.8); color:#fff; border:none; border-radius:4px; padding:2px 6px; font-size:10px; cursor:pointer;">✕</button>
...
function handleFlagUpload() {
    ...
    if (!_flagBase64) return showAlert('Error', 'Please select a flag image.');

    db.ref(`global_nat_flags/${key}`).set({ name, key, image: _flagBase64, aliases }).then(() => {
...
function renderRoleIcons() {
    ...
    keys.forEach(key => {
        ...
        card.style.cssText = 'background:#111; border:1px solid #2d2d3f; border-radius:10px; overflow:hidden; text-align:center; padding:16px 10px; position:relative;';
        card.innerHTML = `
            <button onclick="window.deleteRoleIcon('${esc(key)}')" style="position:absolute; top:6px; right:6px; background:rgba(220,53,69,0.8); color:#fff; border:none; border-radius:4px; padding:2px 6px; font-size:10px; cursor:pointer;">✕</button>
...
function handleRoleIconUpload() {
    ...
    if (!_roleIconBase64) return showAlert('Error', 'Please select an icon image.');

    db.ref(`global_role_icons/${key}`).set({ name, key, image: _roleIconBase64, aliases }).then(() => {
        ...
        _roleIconBase64 = '';
        showAlert('Success', `Role icon for "${name}" (${key}) uploaded successfully!`);
    });
}
