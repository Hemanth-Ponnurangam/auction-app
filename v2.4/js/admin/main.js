...
function renderDatabaseManager() {
    ...
    dbKeys.forEach(dbName => {
        try {
            ...
            const allKeys = [...new Set(filteredPlayers.flatMap(p => Object.keys(p)))].filter(k => k !== 'status');
            const thHtml = allKeys.map(k => `<th style="padding:6px 10px; font-size:10px; color:#ffc107; text-transform:uppercase; letter-spacing:.5px; white-space:nowrap; border-bottom:1px solid #333; border-right:1px solid #1a1a24; background:#0d0d18; position:sticky; top:0;">${esc(k)}</th>`).join('');
            const trHtml = filteredPlayers.map((p, i) => {
                const cells = allKeys.map(k => `<td style="padding:5px 10px; font-size:11px; color:${k==='name'?'#fff':'#aaa'}; border-bottom:1px solid #111; border-right:1px solid #1a1a24; white-space:nowrap;">${esc(String(p[k] ?? ''))}</td>`).join('');
...
function handleDatabaseUpload() {
    ...
    reader.onload = e => {
        ...
        // Stat columns = any header after index 5 (after franchise)
        const CORE_COLS = new Set(['name','player','base_price','price','role','set','nationality','franchise']);
        const statCols = headers.filter(h => h && !CORE_COLS.has(h));
        if (statCols.length > 6) {
            return showAlert('Error', `Too many stat columns (${statCols.length}). Maximum 6 stat columns allowed.`);
        }
        ...
        let pool = rows.slice(1).map(row => {
            ...
            headers.forEach((h, i) => {
                let val = row[i] ? row[i].trim() : '';
                if (h === 'player' || h === 'name') p.name = val;
                else if (h === 'base_price' || h === 'price') p.base_price = parseInt(val.replace(/[^0-9]/g,'')) || 0;
                else if (val !== '') p[h] = val;
            });
            // Store stat columns grouped
            if (statCols.length > 0) {
                p.stats = {};
                statCols.forEach(sc => { if (p[sc] !== undefined) { p.stats[sc] = p[sc]; } });
            }
            return p;
        }).filter(p => p.name);
