...
function updateDbDropdown() {
    ...
    Object.keys(activeDatabases).forEach(k => {
...
window.deleteDatabase = (e, dbName) => {
    e.stopPropagation();
    showConfirm('Delete Database', `Are you sure you want to permanently delete "${dbName}"?`, () => {
        db.ref(`presets/${dbName}`).remove();
    });
};
...
function renderImageCards() {
    ...
    keys.forEach(playerName => {
        ...
        // Data Normalization (Fixes crashes from old Firebase database structures)
        if (typeof rawData === 'string') {
            activeUrl = rawData;
            urlList = [rawData];
        } else if (rawData && rawData.active) {
            activeUrl = rawData.active;
            urlList = rawData.urls || [rawData.active];
        } else if (rawData && rawData.url) {
            activeUrl = rawData.url;
            urlList = [rawData.url];
        } else {
            return; // Corrupted entry, skip it
        }
...
window.addUrlToPlayer = (playerName) => {
    showPrompt('Add Image URL', `Paste new image URL for ${playerName}:`, 'https://...', (url) => {
        ...
        } else if (imgData && imgData.url && !imgData.urls) {
            imgData = { active: imgData.url, urls: [imgData.url] };
        } else if (!imgData || !imgData.urls) {
            imgData = { active: url, urls: [] };
        }
...
window.deleteRoom = (key) => {
    showConfirm('Terminate Room', `Are you sure you want to permanently delete room [${key}]?`, () => {
...
function closeAddTeamModal() {
    ...
    document.getElementById('ntCode').disabled = false; // Release lock if it was edited
    document.getElementById('ntName').value = '';
    
    let logoInput = document.getElementById('ntLogo');
    if (logoInput) logoInput.value = '';
    
    // Reset Modal Text back to "Add" mode
...
window.editGlobalTeam = (code) => {
    ...
    document.getElementById('ntCode').disabled = true; // Prevent changing the primary key
