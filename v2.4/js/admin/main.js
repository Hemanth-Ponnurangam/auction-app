...
function renderDatabaseManager() {
    ...
    dbKeys.forEach(dbName => {
        try {
            ...
            const STANDARD_ORDER = ['name','base_price','role','set','nationality','franchise'];
...
function handleDatabaseUpload() {
    ...
    reader.onload = e => {
        ...
        // Stat columns = anything beyond the 6 standard fields (flexible, max 6)
        ...
        let pool = rows.slice(1).map(row => {
            ...
            headers.forEach((h, i) => {
                ...
                else if (h) p[h] = val;   // store every column directly — no nesting
            });
            return p;
        }).filter(p => p.name);
