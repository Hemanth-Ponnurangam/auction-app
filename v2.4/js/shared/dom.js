...
export function showConfirm(title, msg, onConfirm, onCancel, confirmLabel = 'CONFIRM', cancelLabel = 'CANCEL') {
    document.getElementById('modalTitle').textContent = title;
    document.getElementById('modalMessage').textContent = msg;
    document.getElementById('modalInput').style.display = 'none';
    let btnDiv = document.getElementById('modalButtons');
    btnDiv.innerHTML = `<button class="action-btn danger" style="flex:1;" id="btnConfOk">${confirmLabel}</button><button class="action-btn outline" style="flex:1;" id="btnConfCancel">${cancelLabel}</button>`;
    document.getElementById('appModal').style.display = 'flex';
    document.getElementById('btnConfOk').onclick = () => { closeModal(); onConfirm(); };
    document.getElementById('btnConfCancel').onclick = () => { closeModal(); if (typeof onCancel === 'function') onCancel(); };
}
