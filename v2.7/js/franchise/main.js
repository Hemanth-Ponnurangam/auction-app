...
window.togglePlayerCardFlip = function() {
    ...
    if (btn) btn.textContent = _cardIsFlipped ? '📷' : '📊';
};
...
function startConfetti(primaryColor) {
    ...
    let colors = [primaryColor || '#ffd700','#fff','#22c55e','#0dcaf0','#ffc107','#f97316'];
    _confettiParticles = Array.from({length: 150}, () => ({
    ...
    function draw() {
        ...
        if (_confettiParticles.length > 0) _confettiRaf = requestAnimationFrame(draw);
...
function triggerSoldAnimation(teamName, teamColor) {
    let overlay = document.getElementById('soldUnsoldOverlay');
    if (!overlay) return;
    overlay.innerHTML = `
        <div class="sold-stamp-wrap">
            <span class="sold-stamp-text">SOLD!</span>
            <span class="sold-team-name" style="color:${teamColor||'#22c55e'};">${esc(teamName)}</span>
        </div>`;
    overlay.className = 'sou-overlay show';
    startConfetti(teamColor);
    setTimeout(() => {
        overlay.className = 'sou-overlay fade-out';
        stopConfetti();
        setTimeout(() => { overlay.className = 'sou-overlay'; }, 700);
    }, 3200);
}
...
function triggerUnsoldAnimation() {
    let overlay = document.getElementById('soldUnsoldOverlay');
    if (!overlay) return;
    overlay.innerHTML = `<div class="unsold-stamp-wrap"><span class="unsold-stamp-text">UNSOLD</span></div>`;
    overlay.className = 'sou-overlay show';
    setTimeout(() => {
        overlay.className = 'sou-overlay fade-out';
        setTimeout(() => { overlay.className = 'sou-overlay'; }, 700);
...
function attachFirebaseListeners() {
    ...
    state.roomRef.child('live_state').on('value', snap => {
        ...
        if (newState === 'sold' && prevState !== 'sold') {
            ...
            triggerSoldAnimation(data.highest_bidder, winnerColor);
        }
...
function updateLiveUI(data, amIWinning) {
    ...
    if (pIdx >= 0 && state.playerPool.length > 0 && data.auction_state !== 'idle') {
        ...
        if (p) {
            ...
            // National flag badge
            let flagEl = document.getElementById('playerFlagBadge');
            if (flagEl) flagEl.textContent = getNatFlag(p.nationality);
